...
def neh_plot2d(datas,pd,grid,xlabel,ylabel,title,axes,sub,size,leg_labels,ax):
...
def neh_subplot2d(plots,bigtitle,shape,size): 
    ...
    for i in range(1,Nplots+1):
        ...
        neh_plot2d(plots[i-1]['datas'],plots[i-1]['pd'],plots[i-1]['grid'],plots[i-1]['xlabel'],plots[i-1]['ylabel'],plots[i-1]['title'],plots[i-1]['axes'],1,[1,1],plots[i-1]['leg_labels'],ax)
...
def neh_contourplot(dictionary, X_key, Y_key, Z_key, color_lims, color_map, xlabel='xlabel', ylabel='ylabel', title='title', size=[10, 7], sub=0):
    ...
    X, Y = nplt.meshgrid(dictionary[X_key], dictionary[Y_key])
